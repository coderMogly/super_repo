...
*/
  ...
  int create_gpa_plan_astar(const COSTTYPE *costmap, int nx, int ny, int* goal, int* start, float *plan, int nplan);
  ...
  class GPAFN
  {
    public:
       ...
       */
      GPAFN(int nx, int ny);	// size of map
       ...
       * @param ny The y size of the map 
       */
      void setGpaArr(int nx, int ny); /**< sets or resets the size of the map */
       ...
       */
      void setCostmap(const COSTTYPE *cmap, bool allow_unknown = true); /**< sets up the cost map */

      /**
       * @brief  Calculates a plan using the A* heuristic, returns true if one is found
       * @return True if a plan is found, false otherwise
       */
      bool calcGPAFNAstar();	/**< calculates a plan, returns true if found */

      /**
       * @brief Caclulates the full navigation function using Dijkstra
       */
      bool calcGPAFNDijkstra(bool atStart = false);	/**< calculates the full navigation function */
      ...
      void setupGPAFN(bool keepit = false); /**< resets all nav fn arrays for propagation */
       ...
       */
      bool propGPAFNDijkstra(int cycles, bool atStart = false); /**< returns true if start point found or full prop */
      /**
       ...
       * @return true if the start point is reached
       */
      bool propGPAFNAstar(int cycles); /**< returns true if start point found */
      ...
      /** display callback */
      void display(void fn(GPAFN *nav), int n = 100); /**< <n> is the number of cycles between updates  */
      int displayInt;		/**< save second argument of display() above */
      void (*displayFn)(GPAFN *nav); /**< display function itself */
