...
int main(int argc, char **argv)
{
  ...
  if (cmap)
  ...
  else
    {
      ...
      sy = sx;
      nav = new GPAFN(sx,sy); // size in pixels
      goal[0] = sx-10;
      goal[1] = sy/2;
      start[0] = 20;
      start[1] = sy/2;
  ...
  // display
  nwin = new GpaWin(sx,sy,"Potential Field");
  ...
  if (cmap)
    ...
    {
      nav->setupGPAFN(false);
      nav->setObs();		// simple obstacles
    }
  double t1 = get_ms();
  //  nav->calcNavFnAstar();
  nav->calcGPAFNDijkstra(true);
  double t2 = get_ms();
  ...
  printf("[GPAFN] Cells not touched: %d/%d\n", ntot, nav->nx*nav->ny);
