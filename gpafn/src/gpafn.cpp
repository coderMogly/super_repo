...
namespace GpaFN {
  ...
  int create_gpa_plan_astar(COSTTYPE *costmap, int nx, int ny, int* goal, int* start, float *plan, int nplan)
    {
      static GPAFN *gpa = NULL;

      if (gpa == NULL)
        gpa = new GPAFN(nx,ny);

      if (gpa->nx != nx || gpa->ny != ny) // check for compatibility with previous call
      {
        delete gpa;
        gpa = new GPAFN(nx,ny);      
      }
      ...
      gpa->priInc = 2*COST_NEUTRAL;
      gpa->propGPAFNAstar(std::max(nx*ny/20,nx+ny));

      // path
      int len = gpa->calcPath(nplan);

      if (len > 0)			// found plan
        ROS_DEBUG("[GPAFN] Path found, %d steps\n", len);
      else
        ROS_DEBUG("[GPAFN] No path found\n");

      if (len > 0)
      {
        for (int i=0; i<len; i++)
        {
          plan[i*2] = gpa->pathx[i];
          plan[i*2+1] = gpa->pathy[i];
        }
      }
  ...
  void
    GPAFN::setGoal(int *g)
    {
      goal[0] = g[0];
      goal[1] = g[1];
      ROS_DEBUG("[GPAFN] Setting goal to %d,%d\n", goal[0], goal[1]);
    }

  void
    GPAFN::setStart(int *g)
    {
      start[0] = g[0];
      start[1] = g[1];
      ROS_DEBUG("[GPAFN] Setting start to %d,%d\n", start[0], start[1]);
    }
  ...
  void
    GPAFN::setGpaArr(int xs, int ys)
    {
      ROS_DEBUG("[GPAFN] Array is %d x %d\n", xs, ys);
  ...
  bool GPAFN::calcGPAFNDijkstra(bool atStart)
    {
      ...
      // calculate the nav fn and path
      propGPAFNDijkstra(std::max(nx*ny/20,nx+ny),atStart);
  ...
  bool GPAFN::calcGPAFNAstar()
    {
      setupGPAFN(true);

      // calculate the nav fn and path
      propGPAFNAstar(std::max(nx*ny/20,nx+ny));
  ...
  float *GPAFN::getPathX() { return pathx; }
  float *GPAFN::getPathY() { return pathy; }
  int    GPAFN::getPathLen() { return npath; }
...
#define push_over(n) { if (n>=0 && n<ns && !pending[n] && \
  ...
  void GPAFN::setupGPAFN(bool keepit)
    {
...
#define INVSQRT2 0.707106781
  ...
  inline void GPAFN::updateCell(int n)
    {
      // get neighbors
      float u,d,l,r;
      l = potarr[n-1];
...
#define INVSQRT2 0.707106781
  ...
  inline void GPAFN::updateCellAstar(int n)
    {
      // get neighbors
      float u,d,l,r;
      l = potarr[n-1];
  ...
  bool GPAFN::propGPAFNDijkstra(int cycles, bool atStart)	
    {
      ...
      ROS_DEBUG("[GPAFN] Used %d cycles, %d cells visited (%d%%), priority buf max %d\n", 
  ...
  bool GPAFN::propGPAFNAstar(int cycles)	
    {
      int nwv = 0;			// max priority block size
      int nc = 0;			// number of cells put into priority blocks
      int cycle = 0;		// which cycle we're on
  ...
  int GPAFN::calcPath(int n, int *st)
    {
      // test write
      //savemap("test");
      ...
      ROS_DEBUG("[GPA PathCalc] No path found, path too long");
  ...
  void GPAFN::display(void fn(GPAFN *nav), int n)
    {
      displayFn = fn;
      displayInt = n;
    }
  ...
  void GPAFN::savemap(const char *fname)
    {
      char fn[4096];

      ROS_DEBUG("[GPAFN] Saving costmap and start/goal points");
